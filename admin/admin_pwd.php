...
if($act=="save")
{
	$sql="update admin set name='$Username',pwd='$Password_new' where id=$id";
	echo "<div>$sql</div>";
 $result=mysqli_query($conn,$sql);
	if($result){
	echo "<SCRIPT LANGUAGE='JavaScript'>alert('管理员账号修改成功');</SCRIPT>";	
	}
	else{
echo "<SCRIPT LANGUAGE='JavaScript'>alert('管理员账号修改成功');</SCRIPT>";	
	}
...
document.form1.submit();
  ...
  <table width="97%" border="0" align="center" cellpadding="3" cellspacing="1" class="tableBorder">
    ...
    <tr>
      ...
      <td width="84%" colspan="2" class="table"><input name="Username" type="text" id="UserName3" value="<?php echo $data['name']?>" readonly></td>
    </tr>
